 ...
 * @returns {Object} Last row data or null if not found
 */
 ...
 * @returns {Object} Response with trading history
 */
 ...
 * @param {number} startBalance - Starting balance for the day
 * @param {number} dailyProfit - Daily profit/loss
 * @param {number} lotSize - Lot size used
 * @param {string} notes - Optional notes
 * @param {string} tradeDate - Trade date (YYYY-MM-DD format)
 * @returns {Object} Response with operation result
 */
 ...
 * @returns {Object} Response with trading statistics
 */
 ...
 * @param {number} limit - Number of recent trades to return (default: 10)
 * @param {string} accountId - Optional account ID filter
 * @returns {Object} Response with recent trades
 */
function getRecentTrades(limit = 10, accountId = null) {
    try {
      ...
      const headers = ['Transaction_ID', 'Timestamp', 'Account ID', 'Asset ID',
        'เงินต้นเริ่มต้นวัน (USD)', 'กำไร/ขาดทุนรายวัน (USD)',
        'เงินรวมสิ้นวัน (USD)', 'Lot Size', 'หมายเหตุ', 'Trade Date'];
      let allTrades = convertSheetDataToJSON(values, headers);
 ...
 * @param {string} tradesDataJSON - JSON string containing array of trade data
 * @param {string} token - User authentication token
 * @returns {Object} Response with batch operation results
 */
